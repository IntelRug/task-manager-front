<template>
  <div class="main-content">
    <div class="content-page">
      <div class="content__header-panel"></div>
      <div class="main-page">
        <div class="main-page__menu main-page__menu_left">
          <div class="left-menu__block">
            <div class="left-menu__block-header material-block">
              Создать новый список <i class="fas fa-plus"></i>
            </div>
            <ul class="left-menu__block-items">
              <li class="left-menu__block-item">Фильмы</li>
              <li class="left-menu__block-item">Книги</li>
              <li class="left-menu__block-item">Список 1</li>
              <li class="left-menu__block-item">Список 2</li>
              <li class="left-menu__block-item">Список 3</li>
              <li class="left-menu__block-item">Список 4</li>
            </ul>
          </div>
          <div class="left-menu__block">
            <div class="left-menu__block-header material-block">
              Создать новую задачу <i class="fas fa-plus"></i>
            </div>
            <ul class="left-menu__block-items">

            </ul>
          </div>
        </div>
        <div class="main-page__container">
          <div class="main-page__task-list">
            <div class="task-list__header material-block">
              <div class="task-list__header-create no-select">Создать задачу</div>
              <div class="task-list__header-sort no-select"
                   v-on:click="triggerSort">
                <span v-html="sortHtmlCurrent"></span>
                <div v-if="sortOpened" class="header-sort__list">
                  <div v-for="n in sortHtmlAll.length"
                       :key="n"
                       v-on:click="sort(n - 1)"
                       v-html="sortHtmlAll[n - 1]"
                       class="header-sort__list-item"></div>
                </div>
              </div>
            </div>
            <div class="task-list__content material-block">
              <ul class="task-list__task-items">
                <router-link tag="li" :to="taskLink(task)" v-for="task in tasks" :key="task.id"
                             class="task-list__task-item">
                  <div
                    class="task-item__favorites no-select">
                    <i class="far fa-star" :class="{select: task.important}"></i>
                  </div>
                  <div class="task-item__name">{{task.name}}</div>
                  <div :class="taskStatus(task).class"
                       class="task-item__status">
                    {{taskStatus(task).text}}
                  </div>
                  <div class="task-item__author">{{taskExecutorsString(task)}}</div>
                </router-link>
              </ul>
            </div>
          </div>
        </div>
        <router-view name="right"></router-view>
      </div>
    </div>
  </div>
</template>

<script>
import { mapGetters } from 'vuex';

export default {
  name: 'App',
  data() {
    return {
      sortType: 3,
      sortOpened: false,
      sortHtmlAll: [
        '<i class="fas fa-sort-amount-down"></i> По названию',
        '<i class="fas fa-sort-amount-up"></i> По названию',
        '<i class="fas fa-sort-amount-down"></i> По дате добавления',
        '<i class="fas fa-sort-amount-up"></i> По дате добавления',
      ],
    };
  },
  watch: {
    sortType() {
      this.sort(this.sortType);
    },
  },
  computed: {
    ...mapGetters(['tasks']),
    taskStatus() {
      return ({ status }) => {
        const texts = [
          'Новое',
          'В работе',
          'Проверяется',
          'Завершено',
        ];
        return {
          class: {
            new: status === 0,
            inwork: status === 1,
            verifying: status === 2,
            completed: status === 3,
          },
          // eslint-disable-next-line no-nested-ternary
          text: texts[status],
        };
      };
    },
    sortHtmlCurrent() {
      return this.sortHtmlAll[this.sortType];
    },
    taskLink() {
      return ({ id }) => (this.$route.fullPath === `/tasks/${id}` ? '/' : `/tasks/${id}`);
    },
    taskExecutorsString() {
      return ({ executors }) => executors.map(e => `${e.first_name} ${e.last_name}`).join(', ');
    },
  },
  methods: {
    sort(type) {
      this.sortType = type;
      switch (type) {
        case 0:
          this.tasks.sort((a, b) => (a.name < b.name ? 1 : -1));
          break;
        case 1:
          this.tasks.sort((a, b) => (a.name > b.name ? 1 : -1));
          break;
        case 2:
          this.tasks.sort((a, b) => (a.created_at > b.created_at ? 1 : -1));
          break;
        case 3:
          this.tasks.sort((a, b) => (a.created_at < b.created_at ? 1 : -1));
          break;
        default:
      }
    },
    triggerSort() {
      this.sortOpened = !this.sortOpened;
    },
  },
  async created() {
    await this.$store.dispatch('GET_TASKS');
    await this.$nextTick();
    this.sort(this.sortType);
  },
};
</script>

<style scoped>

</style>
